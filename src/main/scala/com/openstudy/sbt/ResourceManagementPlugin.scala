package com.openstudy { package sbt {
  ...
  object ResourceManagementPlugin extends Plugin
    ...
    val resourceManagementSettings = Seq(
      ...
      awsSecretKey := None,
      awsS3Bucket := None,
      forceSassCompile in ResourceCompile := false,
      productionSassCompile in ResourceCompile := true,
      gzipResources := false,
      ...
      compileSass in ResourceCompile <<= (streams, baseDirectory, awsS3Bucket, forceSassCompile in ResourceCompile, productionSassCompile in ResourceCompile) map doSassCompile _,
